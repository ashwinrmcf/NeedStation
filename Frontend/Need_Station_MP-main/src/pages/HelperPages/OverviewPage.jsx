...
const OverviewPage = () => {
	...
	return (
		<div className='flex-1 flex flex-col h-full'>
			...
			{/* Content area - Optimized for mobile/tablet */}
			<main className='flex-1 py-4 px-3 sm:py-6 sm:px-4 lg:px-8 max-w-7xl mx-auto w-full overflow-x-hidden'>
				...
				<AnimatePresence>
					{isWelcomeCardVisible ? (
						...
						>
							<Eye size={18} />
							<span>{t("showWelcomeCard")}</span>
						</motion.button>
					)}
				</AnimatePresence>
				
				{/* MY TASKS - Mobile optimized */}
				<motion.div 
				...
				>
					<div className='flex items-center gap-2 mb-3 sm:mb-4'>
						<ClipboardList className='text-[#00E0B8]' size={20} />
						<h2 className='text-lg sm:text-xl font-semibold text-white'>{t("myTasks")}</h2>
					</div>
					<div className='space-y-3'>
						{todaysTasks.map((task) => (
							<div key={task.id} className='bg-gray-700 rounded-lg p-3 sm:p-4 border border-gray-600 hover:border-[#00E0B8] transition-colors'>
								<div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0 mb-3'>
									<div className='flex items-center gap-2 sm:gap-3 flex-1'>
										<div className='bg-[#00E0B8] rounded-full p-1.5 sm:p-2 flex-shrink-0'>
											<Heart className='text-gray-900' size={16} />
										</div>
										<div className='min-w-0 flex-1'>
											<p className='text-white font-medium text-sm sm:text-base truncate'>{task.customerName || t("customer")}</p>
											<p className='text-[#00E0B8] text-xs sm:text-sm font-medium truncate'>{task.serviceName}</p>
										</div>
									</div>
									<div className='text-right flex-shrink-0'>
										<p className='text-green-400 font-bold text-base sm:text-lg'>₹{task.totalAmount}</p>
										<p className='text-gray-400 text-xs'>{task.preferredTime || task.preferredTimeSlot}</p>
									</div>
								</div>
								
								{/* Task Details - Mobile optimized */}
								<div className='grid grid-cols-1 sm:grid-cols-2 gap-2 mt-3 pt-3 border-t border-gray-600'>
				...
				{/* SIMPLE STATS CARDS - Mobile optimized */}
				<motion.div
				...
				>
					<div className='bg-gray-800 rounded-lg p-2.5 sm:p-4 text-center'>
						<CheckCircle className='text-green-400 mx-auto mb-1' size={20} />
						<p className='text-lg sm:text-2xl font-bold text-white'>{stats?.tasksThisWeek || 0}</p>
						<p className='text-gray-400 text-[10px] sm:text-sm leading-tight'>{t("tasksThisWeek")}</p>
					</div>
					<div className='bg-gray-800 rounded-lg p-2.5 sm:p-4 text-center'>
						<DollarSign className='text-green-400 mx-auto mb-1' size={20} />
						<p className='text-lg sm:text-2xl font-bold text-white'>₹{stats?.monthlyEarnings || 0}</p>
						<p className='text-gray-400 text-[10px] sm:text-sm leading-tight'>{t("monthlyEarnings")}</p>
					</div>
					<div className='bg-gray-800 rounded-lg p-2.5 sm:p-4 text-center'>
						<Star className='text-yellow-400 mx-auto mb-1' size={20} />
						<p className='text-lg sm:text-2xl font-bold text-white'>{stats?.averageRating?.toFixed(1) || 0}/5</p>
						<p className='text-gray-400 text-[10px] sm:text-sm leading-tight'>{t("yourRating")}</p>
					</div>
					<div className='bg-gray-800 rounded-lg p-2.5 sm:p-4 text-center'>
						<MessageCircle className='text-blue-400 mx-auto mb-1' size={20} />
						<p className='text-lg sm:text-2xl font-bold text-white'>{stats?.newMessages || 0}</p>
						<p className='text-gray-400 text-[10px] sm:text-sm leading-tight'>{t("newMessages")}</p>
					</div>
				</motion.div>

				{/* QUICK ACTIONS - Mobile optimized */}
				<motion.div
					className='bg-gray-800 rounded-lg p-3 sm:p-6 mb-4 sm:mb-6'
					initial={{ opacity: 0, y: 20 }}
					animate={{ opacity: 1, y: 0 }}
					transition={{ duration: 0.5, delay: 0.4 }}
				>
					<h3 className='text-base sm:text-xl font-semibold text-white mb-2 sm:mb-4 flex items-center gap-2'>
						<Activity className='text-[#00E0B8]' size={18} />
						<span className='text-sm sm:text-base'>{t("quickActions")}</span>
					</h3>
					<div className='grid grid-cols-2 gap-2 sm:gap-3'>
						<button className='bg-[#00E0B8] hover:bg-[#00C4A0] text-gray-900 font-semibold py-2.5 sm:py-3 px-2 sm:px-4 rounded-lg flex flex-col items-center justify-center gap-1 transition-colors'>
							<MapPin size={16} />
							<span className='text-[10px] sm:text-sm leading-tight'>{t("markLocation")}</span>
						</button>
						<button className='bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 sm:py-3 px-2 sm:px-4 rounded-lg flex flex-col items-center justify-center gap-1 transition-colors'>
							<Phone size={16} />
							<span className='text-[10px] sm:text-sm leading-tight'>{t("callSupport")}</span>
						</button>
						<button className='bg-green-600 hover:bg-green-700 text-white font-semibold py-2.5 sm:py-3 px-2 sm:px-4 rounded-lg flex flex-col items-center justify-center gap-1 transition-colors'>
							<CheckCircle size={16} />
							<span className='text-[10px] sm:text-sm leading-tight'>{t("completeTask")}</span>
						</button>
						<button className='bg-red-600 hover:bg-red-700 text-white font-semibold py-2.5 sm:py-3 px-2 sm:px-4 rounded-lg flex flex-col items-center justify-center gap-1 transition-colors'>
							<AlertTriangle size={16} />
							<span className='text-[10px] sm:text-sm leading-tight'>{t("reportIssue")}</span>
						</button>
					</div>
				</motion.div>

				{/* HEALTH REMINDERS - Mobile optimized */}
				<motion.div
				...
				>
					<h3 className='text-lg sm:text-xl font-semibold text-white mb-3 sm:mb-4 flex items-center gap-2'>
						<Shield className='text-[#00E0B8]' size={20} />
