...
import { FaStar, FaClipboardList, FaVial, FaUtensils, FaFileUpload, FaEnvelope } from "react-icons/fa";
import styles from "../BasicNeeds/BasicNeedsServiceUserDescription.module.css";

const HealthCheckupUniqueDetails = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submitMessage, setSubmitMessage] = useState("");
  
  const FORM_DATA_KEY = 'needstation_healthcheckup_form_data';
    ...
    try {
      const response = await fetch('http://localhost:8080/api/user/update-form-data', {
        ...
        body: JSON.stringify({
          ...
          testsRequired: formData.testsRequired.join(', '),
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          navigate("/available-helpers", {
            state: { 
              service: "Health Check-Up Services",
    ...
    <div style={{
      ...
      <div className={styles["box"]} style={{
        ...
        <h1 style={{
          ...
          <FaClipboardList style={{ marginRight: "15px", fontSize: "28px" }} />
          Type of Package
        </h1>
        <div style={{ 
          display: "grid", 
          ...
          {['Basic', 'Executive', 'Senior Citizen', 'Women\'s Health', 'Disease-Specific'].map((pkg) => (
            <label key={pkg} style={{
              display: "flex",
              alignItems: "center",
              color: "white",
              ...
              backgroundColor: formData.packageType === pkg.toLowerCase() ? "rgba(92, 225, 230, 0.2)" : "rgba(255, 255, 255, 0.05)",
              border: formData.packageType === pkg.toLowerCase() ? "2px solid rgba(92, 225, 230, 0.5)" : "2px solid transparent",
              minHeight: "50px",
              ...
              <input
                ...
                checked={formData.packageType === pkg.toLowerCase()}
      ...
      <div className={styles["box"]} style={{
        ...
        <div style={{ 
          ...
          {['CBC', 'ECG', 'Lipid Profile', 'Thyroid', 'Blood Sugar', 'Kidney Function'].map((test) => (
            <label key={test} style={{
              ...
              borderRadius: "12px",
              transition: "all 0.3s ease",
              backgroundColor: formData.testsRequired.includes(test.toLowerCase()) ? "rgba(92, 225, 230, 0.2)" : "rgba(255, 255, 255, 0.05)",
              border: formData.testsRequired.includes(test.toLowerCase()) ? "2px solid rgba(92, 225, 230, 0.5)" : "2px solid transparent",
              minHeight: "50px",
              justifyContent: "flex-start"
            }}>
              <input
                type="checkbox"
                checked={formData.testsRequired.includes(test.toLowerCase())}
                onChange={() => handleCheckboxChange('testsRequired', test.toLowerCase())}
                style={{ 
                  marginRight: "15px", 
                  accentColor: "#5CE1E6",
                  width: "18px",
      ...
      <div className={styles["box"]} style={{
        ...
        <textarea
          ...
          onChange={handleInputChange}
          placeholder="Please provide any previous health records, medical history, or specific concerns..."
          rows="4"
          style={{
            width: "100%",
            padding: "18px 20px",
      ...
      <div className={styles["button-container"]} style={{
        ...
        <button 
          ...
          disabled={!formData.packageType || formData.testsRequired.length === 0 || !formData.fastingRequired || !formData.reportDelivery || isSubmitting}
          onClick={async (e) => {
            e.preventDefault();
            
            if (!formData.packageType || formData.testsRequired.length === 0 || !formData.fastingRequired || !formData.reportDelivery) {
              alert("Please fill in all required health check-up details");
              return;
            }
          ...
          style={{
            background: (!formData.packageType || formData.testsRequired.length === 0 || !formData.fastingRequired || !formData.reportDelivery || isSubmitting) 
              ? "linear-gradient(135deg, #666, #888)" 
              : "linear-gradient(135deg, #5CE1E6, #00d4ff)",
            color: (!formData.packageType || formData.testsRequired.length === 0 || !formData.fastingRequired || !formData.reportDelivery || isSubmitting) 
              ? "#ccc" 
              : "#000",
            border: "none",
            borderRadius: "15px",
            ...
            fontSize: "18px",
            fontWeight: "700",
            cursor: (!formData.packageType || formData.testsRequired.length === 0 || !formData.fastingRequired || !formData.reportDelivery || isSubmitting) ? 'not-allowed' : 'pointer',
            transition: "all 0.3s ease",
            boxShadow: (!formData.packageType || formData.testsRequired.length === 0 || !formData.fastingRequired || !formData.reportDelivery || isSubmitting) 
              ? "none" 
