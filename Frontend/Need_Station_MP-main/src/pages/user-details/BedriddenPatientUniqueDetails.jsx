...
import { FaStar, FaBed, FaClock, FaHandsHelping, FaStethoscope, FaFileUpload } from "react-icons/fa";
import styles from "../BasicNeeds/BasicNeedsServiceUserDescription.module.css";

const BedriddenPatientUniqueDetails = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submitMessage, setSubmitMessage] = useState("");
  
  const FORM_DATA_KEY = 'needstation_bedridden_form_data';
    ...
    try {
      const response = await fetch('http://localhost:8080/api/user/update-form-data', {
        ...
        body: JSON.stringify({
          ...
          bedriddenDuration: formData.bedriddenDuration,
          supportNeeded: formData.supportNeeded.join(', '),
          specialMedicalNeeds: formData.specialMedicalNeeds.join(', '),
    ...
    <div style={{
      ...
      <div className={styles["box"]} style={{
        ...
        <div style={{ 
          ...
          {['Stroke', 'Post-Surgery', 'Paralysis', 'Chronic Illness'].map((condition) => (
            <label key={condition} style={{
              ...
              backgroundColor: formData.primaryCondition === condition.toLowerCase() ? "rgba(92, 225, 230, 0.2)" : "rgba(255, 255, 255, 0.05)",
              border: formData.primaryCondition === condition.toLowerCase() ? "2px solid rgba(92, 225, 230, 0.5)" : "2px solid transparent",
              minHeight: "50px",
              ...
              <input
                ...
                checked={formData.primaryCondition === condition.toLowerCase()}
      ...
      <div className={styles["box"]} style={{
        ...
        <div style={{ 
          ...
          {['Short-term', 'Long-term', 'Permanent'].map((duration) => (
            <label key={duration} style={{
              ...
              borderRadius: "12px",
              transition: "all 0.3s ease",
              backgroundColor: formData.bedriddenDuration === duration.toLowerCase() ? "rgba(92, 225, 230, 0.2)" : "rgba(255, 255, 255, 0.05)",
              border: formData.bedriddenDuration === duration.toLowerCase() ? "2px solid rgba(92, 225, 230, 0.5)" : "2px solid transparent",
              minHeight: "50px",
              justifyContent: "flex-start"
            }}>
              <input
                type="radio"
                name="bedriddenDuration"
                value={duration.toLowerCase()}
                checked={formData.bedriddenDuration === duration.toLowerCase()}
                onChange={handleInputChange}
                style={{ 
                  marginRight: "15px", 
                  accentColor: "#5CE1E6",
      ...
      <div className={styles["box"]} style={{
        ...
        <div style={{ 
          ...
          {['Feeding', 'Diaper Change', 'Hygiene', 'Repositioning'].map((support) => (
            <label key={support} style={{
              ...
              borderRadius: "12px",
              transition: "all 0.3s ease",
              backgroundColor: formData.supportNeeded.includes(support.toLowerCase()) ? "rgba(92, 225, 230, 0.2)" : "rgba(255, 255, 255, 0.05)",
              border: formData.supportNeeded.includes(support.toLowerCase()) ? "2px solid rgba(92, 225, 230, 0.5)" : "2px solid transparent",
              minHeight: "50px",
              justifyContent: "flex-start"
            }}>
              <input
      ...
      <div className={styles["box"]} style={{
        ...
        <h1 style={{
          ...
          <FaBed style={{ marginRight: "15px", fontSize: "28px" }} />
          Special Medical Needs
        </h1>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "15px" }}>
          {['Catheter', 'Oxygen', 'IV Line', 'None'].map((need) => (
            <label key={need} style={{
              ...
              borderRadius: "10px",
              transition: "all 0.3s ease",
              backgroundColor: formData.specialMedicalNeeds.includes(need.toLowerCase()) ? "rgba(92, 225, 230, 0.2)" : "transparent"
            }}>
              <input
                type="checkbox"
                checked={formData.specialMedicalNeeds.includes(need.toLowerCase())}
                onChange={() => handleCheckboxChange('specialMedicalNeeds', need.toLowerCase())}
      ...
      <div className={styles["box"]} style={{
        ...
        <textarea
          ...
          onChange={handleInputChange}
          placeholder="Please provide doctor's notes, prescriptions, or any special care instructions..."
          rows="4"
          style={{
            width: "100%",
            padding: "18px 20px",
      ...
      <div className={styles["button-container"]} style={{
        ...
        <button 
          ...
          disabled={!formData.primaryCondition || !formData.bedriddenDuration || formData.supportNeeded.length === 0 || isSubmitting}
          onClick={async (e) => {
            e.preventDefault();
            
            if (!formData.primaryCondition || !formData.bedriddenDuration || formData.supportNeeded.length === 0) {
              alert("Please fill in all required bedridden patient care details");
              return;
            }
          ...
          style={{
            background: (!formData.primaryCondition || !formData.bedriddenDuration || formData.supportNeeded.length === 0 || isSubmitting) 
              ? "linear-gradient(135deg, #666, #888)" 
              : "linear-gradient(135deg, #5CE1E6, #00d4ff)",
            color: (!formData.primaryCondition || !formData.bedriddenDuration || formData.supportNeeded.length === 0 || isSubmitting) 
              ? "#ccc" 
              : "#000",
            border: "none",
            borderRadius: "15px",
            ...
            fontSize: "18px",
            fontWeight: "700",
            cursor: (!formData.primaryCondition || !formData.bedriddenDuration || formData.supportNeeded.length === 0 || isSubmitting) ? 'not-allowed' : 'pointer',
            transition: "all 0.3s ease",
            boxShadow: (!formData.primaryCondition || !formData.bedriddenDuration || formData.supportNeeded.length === 0 || isSubmitting) 
              ? "none" 
